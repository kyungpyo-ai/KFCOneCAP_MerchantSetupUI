...
// ==============================================================
// [ModernUI.cpp]
//  - ModernUI.h 에 선언된 커스텀 컨트롤/유틸의 구현부
//
// 큰 흐름
//  1) ModernUIGfx  : GDI+ 초기화/종료 + 공용 라운드사각형 Path 생성
//  2) ModernUIDpi  : HWND 기준 DPI 스케일 함수
//  3) ModernUITheme: 인풋(에딧/콤보) 테마 토큰을 런타임에 교체 가능
//  4) 각 컨트롤 클래스:
//     - WM_PAINT/WM_MOUSEMOVE/WM_LBUTTONDOWN/WM_LBUTTONUP/WM_KILLFOCUS 등에서
//       상태(hover/focus/pressed)를 갱신하고 Invalidate()로 재그림
//
// 주의사항
//  - GDI Object 선택/해제, GDI+ Graphics 사용 범위를 벗어나지 않도록 유지
//  - 드로잉은 더블버퍼(CMemDC 등) 사용 전제를 유지(깜빡임/잔상 방지)
// ==============================================================
...
// --------------------------------------------------------------
// [Theme]
//  - 에딧/콤보 공통 보더(일반/호버/포커스)와 라운드/두께 등을 한곳에서 관리
//  - 다이얼로그별/스킨별로 SetInputTheme()로 교체할 수 있게 설계
// --------------------------------------------------------------
...
// --------------------------------------------------------------
// [GDI+ / 공용 그래픽 유틸]
//  - EnsureGdiplusStartup(): 최초 1회 GDI+ Startup
//  - ShutdownGdiplus(): 앱 종료 시 정리 (App::ExitInstance에서 호출)
//  - AddRoundRect(): 라운드 사각형 Path를 생성하는 공용 함수
// --------------------------------------------------------------
...
/*
[호버 상태 업데이트]
- 마우스 이동 시 호버 대상(버튼/아이콘/카드)을 판별해 상태를 갱신합니다.
- 상태가 바뀌면 InvalidateRect()로 최소 영역만 다시 그리도록 최적화하세요.
*/
...
/*
[클릭 시작]
- 클릭한 위치의 UI 요소(버튼/카드/아이콘)를 판별하고 pressed 상태를 설정합니다.
- 캡처(SetCapture) 사용 시 OnLButtonUp에서 반드시 ReleaseCapture 처리하세요.
*/
...
/*
[클릭 완료]
- pressed 상태 해제 후, 클릭 확정 동작(선택/토글/팝오버 오픈)을 수행합니다.
*/
...
{
	if (nHitTest == HTCLIENT && ::IsWindowEnabled(m_hWnd))
	{
		// IDC_HAND: 손가락 모양 커서
		// IDC_ARROW: 일반 화살표 (원하시면 이걸로 바꾸셔도 됩니다)
		::SetCursor(::LoadCursor(NULL, IDC_HAND));
		return TRUE;
	}
...
void CSkinnedComboBox::PaintComboToDC(CDC& dc)
{
	...
	// 2. 배경 그리기
	if (m_bUseUnderlayBg) memDC.FillSolidRect(&rc, m_clrUnderlayBg);
	else kftc_fill_parent_bg(m_hWnd, memDC.GetSafeHdc(), rc);
	...
	Gdiplus::Color bc;
	float thick = 1.2f;
	if (!enabled) bc = Gdiplus::Color(255, GetRValue(KFTC_DISABLED_BORDER), GetGValue(KFTC_DISABLED_BORDER), GetBValue(KFTC_DISABLED_BORDER));
	else if (isActive) { bc = Gdiplus::Color(255, 0, 100, 221); thick = 1.6f; }
	else if (m_bHover) { bc = Gdiplus::Color(255, GetRValue(th.borderH), GetGValue(th.borderH), GetBValue(th.borderH)); thick = 2.0f; }
	else { bc = Gdiplus::Color(255, GetRValue(th.borderN), GetGValue(th.borderN), GetBValue(th.borderN)); }

	if (isActive) {
		Gdiplus::RectF rfGlow = rfOuter; rfGlow.Inflate(1.5f, 1.5f);
		Gdiplus::GraphicsPath pathGlow; AddRoundRect(pathGlow, rfGlow, (Gdiplus::REAL)m_nRadius + 1.2f);
		g.FillPath(&Gdiplus::SolidBrush(Gdiplus::Color(40, 0, 100, 221)), &pathGlow);
	}
	...
	g.FillPath(&Gdiplus::SolidBrush(bgColor), &pathOuter);

	if (isActive) {
		Gdiplus::Pen pen(bc, thick); pen.SetLineJoin(Gdiplus::LineJoinRound);
	...
	float dropW = (float)m_nDropW;
	{
		...
		if (!text.IsEmpty()) {
			...
			float fixedTextMargin = 2.5f;
			rfText.X += (padL + fixedTextMargin);
			rfText.Width -= (padL + dropW + (fixedTextMargin * 2.0f));
		...
		Gdiplus::Pen divPen((!enabled) ? Gdiplus::Color(255, 210, 215, 225) : Gdiplus::Color(255, 230, 235, 245), 1.0f);
...
- 깜빡임 방지를 위해 메모리 DC(더블 버퍼) 사용 여부를 확인하세요.
*/
void CSkinnedComboBox::OnPaint()
{
    /* [UI-STEP] 스킨 콤보 커스텀 페인팅(보더/드롭 화살표)
     * 1) 배경/보더를 테마 기준으로 그리고, 드롭 버튼 영역(화살표)을 별도 스타일로 처리한다.
     * 2) 포커스/hover 상태에 따라 보더 색을 변경한다.
     * 3) 기본 콤보가 그리는 잔상과 충돌하지 않게 클리핑/더블버퍼를 고려한다.
     */
...
void CSkinnedComboBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
     ...
     * 1) lpDIS->itemID로 드랍리스트 항목/에딧 영역을 구분한다.
     * 2) 선택/hover/disabled 상태에 따라 배경색과 텍스트색을 선택한다.
     * 3) 항목 텍스트를 패딩을 두고 좌측 정렬로 출력한다(긴 텍스트는 ellipsis).
     */
...
{
    /* [UI-STEP] 스킨 에딧 서브클래싱 초기화(기본 스타일 보정)
     * 1) 기본 CEdit 스타일/여백이 현대 UI와 다르면 여기서 보정한다.
     * 2) 멀티라인/센터 정렬 등 필요한 스타일을 EnsureMultilineForVCenter()로 맞춘다.
     * 3) 테마 적용(ApplyThemeAndMargins)으로 내부 패딩/보더 규칙을 일관화한다.
     */
...
{
    /* [UI-STEP] 스킨 에딧 테마/마진 적용(입력 박스 통일감 핵심)
     * 1) 현재 활성 테마(ModernUITheme::GetInputTheme 등)를 가져온다.
     * 2) 내부 텍스트 여백(좌/우/상/하)을 DPI에 맞게 산출한다.
     * 3) EM_SETMARGINS 등 메시지로 에딧 내부 패딩을 적용한다.
     * 4) 포커스/hover 시 보더를 그리기 위한 상태 변수를 초기화한다.
     *
     * [참고]
...
{
    /* [UI-STEP] 스킨 에딧 배경 지우기(깜빡임 방지)
     * 1) 배경을 OnPaint에서 완전히 그린다면 TRUE를 리턴해 기본 지우기를 막는다.
     * 2) 이 경우 OnPaint에서 배경을 항상 덮어써야 잔상이 남지 않는다.
     */
...
void CSkinnedEdit::OnPaint()
{
	...
	const BOOL enabled = (::IsWindowEnabled(m_hWnd) != FALSE);
	const KFTCInputTheme& th = GetActiveInputTheme();
	const bool isActive = enabled && m_bFocus;
	...
	// 4. [핵심] 그리기 순서 분기
	// isActive일 때는 테두리를 먼저 그려야 Glow와 자연스럽게 연결됩니다.
	if (isActive)
	{
		Gdiplus::Pen penBorder(bc, thick);
		penBorder.SetLineJoin(Gdiplus::LineJoinRound);
		g.DrawPath(&penBorder, &pathOuter);
	}

	// 5. 네이티브 텍스트 영역 (WM_PRINTCLIENT)
	int save = memDC.SaveDC();
	{
		float clipInset = thick + 0.5f;
		if (isActive) clipInset = thick + 1.2f; // isActive일 때는 클리핑을 더 깊게 넣어 테두리 보호

		int l = (int)(rfOuter.X + clipInset), t = (int)(rfOuter.Y + clipInset);
		int r = (int)(rfOuter.GetRight() - clipInset), b = (int)(rfOuter.GetBottom() - clipInset);
		int rr = (int)m_nRadius - (int)thick; if (rr < 1) rr = 1;
	...
	// 6. [핵심] Normal/Hover일 때만 테두리를 나중에 덧그려 깨짐 방지
	if (!isActive)
	{
		Gdiplus::Pen penBorder(bc, thick);
		penBorder.SetLineJoin(Gdiplus::LineJoinRound);
		// Inset을 사용하지 않고 Center 정렬로 복구하여 어색함 제거
		g.DrawPath(&penBorder, &pathOuter);
	}
...
{
    /* [UI-STEP] hover 상태 추적(마우스 진입/이탈 감지)
     * 1) 마우스가 에딧 위로 들어오면 hover 플래그를 세팅한다.
     * 2) TrackMouseEvent로 WM_MOUSELEAVE를 요청해 이탈 시점을 받는다.
     * 3) hover 상태 변화 시 Invalidate()로 보더 톤을 즉시 갱신한다.
     */
...
{
    /* [UI-STEP] hover 해제 처리
     * 1) hover 플래그를 해제하고 Invalidate()로 보더를 원래 톤으로 복구한다.
     */
...
{
    /* [UI-STEP] 탭 컨트롤 커스텀 페인팅(탭 배경/선택 표시/아이콘)
     * 1) 전체 배경을 먼저 채워 탭 영역 잔상을 방지한다.
     * 2) 각 탭을 순회하며 DrawTab()으로 탭 버튼을 그린다(선택 탭 강조).
     * 3) 필요 시 DrawIcon()으로 탭 아이콘을 함께 렌더링한다.
     * 4) 마지막에 선택 탭 언더라인/보더 등 강조 요소를 그려 z-order를 맞춘다.
     */
...
void CModernTabCtrl::DrawTab(Graphics& g, int idx, const RectF& rc)
{
	...
	if (m_nCachedTabDpi != curTabDpi || !m_font.GetSafeHandle()) {
		...
		bool bCompact = (::GetSystemMetrics(SM_CYSCREEN) <= 800);
		...
		m_fontBold.CreateFont(-(int)ModernUIDpi::ScaleF(m_hWnd, boldSize), 0, 0, 0, boldWeight, FALSE, FALSE, 0,
			DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_SWISS, ModernUIFont::GetUIFontFace());
...
{
    /* [UI-STEP] 라벨 옆 정보 아이콘(i) 렌더링(hover/pressed 강조)
     * 1) 아이콘 버튼은 매우 작기 때문에 히트 영역과 실제 그리기 영역을 동일하게 유지한다.
     * 2) hover/pressed 상태에 따라 배경 원/테두리 톤을 바꾸고, i 글자를 중앙에 렌더링한다.
     * 3) 팝오버를 띄우는 트리거이므로, 클릭 시 시각적 피드백(pressed)을 명확히 준다.
     */
...
/*static*/ void CModernPopover::RegisterPopoverClass()
{
	...
	wc.style = CS_HREDRAW | CS_VREDRAW; // CS_DROPSHADOW :   ( θ) 
...
{
    /* [UI-STEP] 팝오버(도움말 박스) 렌더링(라운드 박스 + 텍스트)
     * 1) 팝오버 배경을 라운드 사각형으로 그리고, 그림자/보더가 있으면 적용한다.
     * 2) 텍스트 영역 패딩을 적용해 줄바꿈이 자연스럽게 보이도록 DrawText 플래그를 사용한다.
     * 3) 부모 좌표 기준으로 표시 위치가 바뀌어도 클리핑되지 않게 창 크기를 보정할 수 있다.
     */
			...
			if (item.bSeparator) {
				...
				if (i == m_nHover) {
					...
					if (item.bDanger) {
						...
						Gdiplus::SolidBrush brHov(Gdiplus::Color(255, 220, 234, 255));
						g.FillPath(&brHov, &hoverPath);
					}
				}
