...
// ==============================================================
// [ModernUI.cpp]
//  - ModernUI.h ¿¡ ¼±¾ðµÈ Ä¿½ºÅÒ ÄÁÆ®·Ñ/À¯Æ¿ÀÇ ±¸ÇöºÎ
//
// Å« Èå¸§
//  1) ModernUIGfx  : GDI+ ÃÊ±âÈ­/Á¾·á + °ø¿ë ¶ó¿îµå»ç°¢Çü Path »ý¼º
//  2) ModernUIDpi  : HWND ±âÁØ DPI ½ºÄÉÀÏ ÇÔ¼ö
//  3) ModernUITheme: ÀÎÇ²(¿¡µ÷/ÄÞº¸) Å×¸¶ ÅäÅ«À» ·±Å¸ÀÓ¿¡ ±³Ã¼ °¡´É
//  4) °¢ ÄÁÆ®·Ñ Å¬·¡½º:
//     - WM_PAINT/WM_MOUSEMOVE/WM_LBUTTONDOWN/WM_LBUTTONUP/WM_KILLFOCUS µî¿¡¼­
//       »óÅÂ(hover/focus/pressed)¸¦ °»½ÅÇÏ°í Invalidate()·Î Àç±×¸²
//
// ÁÖÀÇ»çÇ×
//  - GDI Object ¼±ÅÃ/ÇØÁ¦, GDI+ Graphics »ç¿ë ¹üÀ§¸¦ ¹þ¾î³ªÁö ¾Êµµ·Ï À¯Áö
...
// --------------------------------------------------------------
// [Theme]
//  - ¿¡µ÷/ÄÞº¸ °øÅë º¸´õ(ÀÏ¹Ý/È£¹ö/Æ÷Ä¿½º)¿Í ¶ó¿îµå/µÎ²² µîÀ» ÇÑ°÷¿¡¼­ °ü¸®
...
//  - AddRoundRect(): ¶ó¿îµå »ç°¢Çü Path¸¦ »ý¼ºÇÏ´Â °ø¿ë ÇÔ¼ö
// --------------------------------------------------------------
...
LRESULT CModernButton::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (message == BM_SETSTYLE)
    {
        ...
        // -> ºÎ¸ð DefDlgProc -> xxxSaveDlgFocus -> ÀÚ½Ä ¹öÆ° N°³ ¼øÈ¸ -> O(N^2)
        // µ¿±â SendMessage ¿¬¼â -> Ã¢ ÀüÈ¯ ½Ã "ÀÀ´ä¾øÀ½" ¹ß»ý.
        // CButton::WindowProc °æ·Î¸¦ ¿ìÈ¸ÇÏ°í ½ºÅ¸ÀÏ ºñÆ®¸¸ Á÷Á¢ ±³Ã¼ÇØ ¿¬¼â¸¦ ²÷´Â´Ù.
...
- Ä¸Ã³(SetCapture) »ç¿ë ½Ã OnLButtonUp¿¡¼­ ¹Ýµå½Ã ReleaseCapture Ã³¸®ÇÏ¼¼¿ä.
*/
...
/*
[¹è°æ Áö¿ì±â]
- ±ôºýÀÓÀ» ÁÙÀÌ±â À§ÇØ º¸Åë TRUE¸¦ ¹ÝÈ¯ÇÏ°Å³ª, ÀÚÃ¼ ´õºí¹öÆÛ¿Í ÇÔ²² »ç¿ëÇÕ´Ï´Ù.
...
{
    /* [UI-STEP] ¸ð´ø ¹öÆ° Owner-draw ·»´õ¸µ(»óÅÂº° »ö/º¸´õ/ÅØ½ºÆ®)
     * 1) lpDIS·Î Àü´ÞµÈ »óÅÂ(pressed/disabled/focused)¸¦ ÀÐ¾î ·»´õ¸µ ÅæÀ» °áÁ¤ÇÑ´Ù.
     ...
     *
     * [Âü°í]
     * - Owner-draw´Â WM_DRAWITEM ±â¹ÝÀÌ¶ó, ÄÁÆ®·Ñ ½ºÅ¸ÀÏ(BS_OWNERDRAW)ÀÌ ¼¼ÆÃµÇ¾î¾ß ÇÑ´Ù.
	...
	BOOL isExit     = (t.Find(_T("Ãë¼Ò")) >= 0) || (t.Find(_T("´Ý±â")) >= 0);
	BOOL isDanger   = (t.Find(_T("»èÁ¦")) >= 0);
	BOOL isDownload = (t.Find(_T("´Ù¿î")) >= 0) || (t.Find(_T("Á¶È¸")) >= 0);
...
void CModernToggleSwitch::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
     ...
     * 1) ÇöÀç Ã¼Å© »óÅÂ(GetCheck ¶Ç´Â ³»ºÎ »óÅÂ)¸¦ ÀÐ¾î ON/OFF »öÀ» ¼±ÅÃÇÑ´Ù.
...
/*
[Ä¿½ºÅÒ ÆäÀÎÆÃ]
- ¹è°æ/Ä«µå/±¸ºÐ¼±/ÀÔ·Â º¸´õ µî Modern UI ½ºÅ¸ÀÏÀ» Á÷Á¢ ±×¸³´Ï´Ù.
- ±ôºýÀÓ ¹æÁö¸¦ À§ÇØ ¸Þ¸ð¸® DC(´õºí ¹öÆÛ) »ç¿ë ¿©ºÎ¸¦ È®ÀÎÇÏ¼¼¿ä.
*/
void CSkinnedComboBox::OnPaint()
{
    /* [UI-STEP] ½ºÅ² ÄÞº¸ Ä¿½ºÅÒ ÆäÀÎÆÃ(º¸´õ/µå·Ó È­»ìÇ¥)
     * 1) ¹è°æ/º¸´õ¸¦ Å×¸¶ ±âÁØÀ¸·Î ±×¸®°í, µå·Ó ¹öÆ° ¿µ¿ª(È­»ìÇ¥)À» º°µµ ½ºÅ¸ÀÏ·Î Ã³¸®ÇÑ´Ù.
     * 2) Æ÷Ä¿½º/hover »óÅÂ¿¡ µû¶ó º¸´õ »öÀ» º¯°æÇÑ´Ù.
     * 3) ±âº» ÄÞº¸°¡ ±×¸®´Â ÀÜ»ó°ú Ãæµ¹ÇÏÁö ¾Ê°Ô Å¬¸®ÇÎ/´õºí¹öÆÛ¸¦ °í·ÁÇÑ´Ù.
     */
...
{
    /* [UI-STEP] ½ºÅ² ÄÞº¸ Owner-draw ¾ÆÀÌÅÛ ·»´õ¸µ(µå¶ø¸®½ºÆ® Æ÷ÇÔ)
     * 1) lpDIS->itemID·Î µå¶ø¸®½ºÆ® Ç×¸ñ/¿¡µ÷ ¿µ¿ªÀ» ±¸ºÐÇÑ´Ù.
     * 2) ¼±ÅÃ/hover/disabled »óÅÂ¿¡ µû¶ó ¹è°æ»ö°ú ÅØ½ºÆ®»öÀ» ¼±ÅÃÇÑ´Ù.
     * 3) Ç×¸ñ ÅØ½ºÆ®¸¦ ÆÐµùÀ» µÎ°í ÁÂÃø Á¤·Ä·Î Ãâ·ÂÇÑ´Ù(±ä ÅØ½ºÆ®´Â ellipsis).
     */
...
void CSkinnedEdit::PreSubclassWindow()
{
     ...
     * 3) Å×¸¶ Àû¿ë(ApplyThemeAndMargins)À¸·Î ³»ºÎ ÆÐµù/º¸´õ ±ÔÄ¢À» ÀÏ°üÈ­ÇÑ´Ù.
     */
...
void CSkinnedEdit::ApplyThemeAndMargins()
{
     ...
     * 3) EM_SETMARGINS µî ¸Þ½ÃÁö·Î ¿¡µ÷ ³»ºÎ ÆÐµùÀ» Àû¿ëÇÑ´Ù.
...
{
    /* [UI-STEP] ½ºÅ² ¿¡µ÷ ¹è°æ Áö¿ì±â(±ôºýÀÓ ¹æÁö)
...
void CSkinnedEdit::OnPaint()
{
     ...
     * 4) ÅØ½ºÆ® ±×¸®±â´Â ±âº» ¿¡µ÷ÀÇ Å¬¶óÀÌ¾ðÆ® ±×¸®±â¿Í Ãæµ¹ÇÏÁö ¾Êµµ·Ï PrintClient/WM_PRINTCLIENT µîÀ» È°¿ëÇÑ´Ù.
     *
     * [Âü°í]
     * - ¿¡µ÷Àº ³»ºÎÀûÀ¸·Î ±×¸®´Â ¿µ¿ªÀÌ ÀÖ¾î¼­, º¸´õ¸¦ 'ºÎ¸ð°¡' ±×¸±Áö 'ÄÁÆ®·ÑÀÌ' ±×¸±Áö Á¤Ã¥À» ÅëÀÏÇØ¾ß ÀÜ»óÀÌ ÁÙ¾îµç´Ù.
     */
...
void CSkinnedEdit::OnMouseMove(UINT nFlags, CPoint point)
{
     ...
     * 2) TrackMouseEvent·Î WM_MOUSELEAVE¸¦ ¿äÃ»ÇØ ÀÌÅ» ½ÃÁ¡À» ¹Þ´Â´Ù.
...
void CModernTabCtrl::OnPaint()
{
     ...
     * 4) ¸¶Áö¸·¿¡ ¼±ÅÃ ÅÇ ¾ð´õ¶óÀÎ/º¸´õ µî °­Á¶ ¿ä¼Ò¸¦ ±×·Á z-order¸¦ ¸ÂÃá´Ù.
     */
...
void CModernPopover::OnPaint()
{
     ...
     * 2) ÅØ½ºÆ® ¿µ¿ª ÆÐµùÀ» Àû¿ëÇØ ÁÙ¹Ù²ÞÀÌ ÀÚ¿¬½º·´°Ô º¸ÀÌµµ·Ï DrawText ÇÃ·¡±×¸¦ »ç¿ëÇÑ´Ù.
     * 3) ºÎ¸ð ÁÂÇ¥ ±âÁØÀ¸·Î Ç¥½Ã À§Ä¡°¡ ¹Ù²î¾îµµ Å¬¸®ÇÎµÇÁö ¾Ê°Ô Ã¢ Å©±â¸¦ º¸Á¤ÇÒ ¼ö ÀÖ´Ù.
     */
